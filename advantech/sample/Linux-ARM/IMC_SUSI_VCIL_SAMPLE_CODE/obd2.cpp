...
static IMC_OBD2_MSG_OBJECT last_obd2_message;
...
static void *obd2_read_thread(void *p)
{
	while(thread_enable)
	{
		...
		USHORT result;
		IMC_OBD2_MSG_OBJECT message;
    
		while( (result = SUSI_IMC_OBD2_Read(&message)) == IMC_ERR_NO_ERROR)
		{
			printf("<OBD2,%d>p=%d,%s,dst=%x,src=%x,",
			message.channel_number,
			message.pri,
			message.tat == VCIL_OBD2_TYPE_PHYSICAL ? "PHY" : "FUN",			
			message.dst,
			message.src);
...
void obd2_write()
{
	...
	message.tat = t == 0 ? VCIL_OBD2_TYPE_PHYSICAL : VCIL_OBD2_TAPE_FUNCTIONAL;
	...
	if( (result = SUSI_IMC_OBD2_Write(&message)) != IMC_ERR_NO_ERROR )
	{
		printf("SUSI_IMC_OBD2_Write fail. error code=0x%04x\n", result);
		return;
	}
...
void obd2_quick_send()
{
	...
	if( (result = SUSI_IMC_OBD2_Write(&last_obd2_message)) != IMC_ERR_NO_ERROR )
	{
		printf("SUSI_IMC_OBD2_Write fail. error code=0x%04x\n", result);
		return;
	}
