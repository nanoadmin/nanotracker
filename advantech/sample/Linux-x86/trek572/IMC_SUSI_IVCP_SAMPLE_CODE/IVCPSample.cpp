...
static void show_menu(void)
{
	...
	for (int i = 0; i < IVCP_SAMPLE_CODE_MAX_FUNCTION_PAGE - 1; i++)
	{
		...
		{
			if( page_function_supported[i] == IVCP_SAMPLE_CODE_FUNCTION_DIGITAL_IO)
				continue;
			printf("%d) %s\n", i + 1, page_name[i]);
		}
		else
	...
	while(true)
	{
		...
		switch(op)
		{
			...
			case IVCP_SAMPLE_CODE_FUNCTION_PERIPHERAL_CONTROL:
				peripheral_main();
				break;
			case IVCP_SAMPLE_CODE_FUNCTION_STORAGE:
				storage_main();
				break;
			...
			case IVCP_SAMPLE_CODE_FUNCTION_POWER_MANAGEMENT:
				...
				break;
			case IVCP_SAMPLE_CODE_FUNCTION_TEMPERATURE_SENSOR:
				sensor_main();
				break;
			default:
