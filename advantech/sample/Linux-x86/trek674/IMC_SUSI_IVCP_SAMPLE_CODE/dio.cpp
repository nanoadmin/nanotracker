...
enum page_function{
	...
	SET_VOICE_SOURCE,
	GET_VOICE_SOURCE,
	SET_VOICE_MODULE_SELECT,
	GET_VOICE_MODULE_SELECT
};
...
static void page_menu()
{
	...
	printf("%d) Set Voice-call Module Select\n", SET_VOICE_MODULE_SELECT);
	printf("%d) Get Voice-call Module Select status\n", GET_VOICE_MODULE_SELECT);
...
void get_voice_module_select()
{
	USHORT result;

	int status;

	if( (result = SUSI_IMC_DIO_GetVoiceModuleSelectPin(&status)) != IMC_ERR_NO_ERROR )
	{
		printf("SUSI_IMC_DIO_GetVoiceModuleSelectPin fail. error code=0x%04x\n", result);
		return;
	}

	printf("The Voice-call Module Select %s\n", status ? "MX73X4":"MX809X");
}

void set_voice_module_select()
{
	printf("Voice-cann Module Select:\n");
	printf("0) MX809X\n");
	printf("1) MX73X4\n");
	...
	if( (result = SUSI_IMC_DIO_SetVoiceModuleSelectPin(e)) != IMC_ERR_NO_ERROR )
	{
		printf("SUSI_IMC_DIO_SetVoiceModuleSelectPin fail. error code=0x%04x\n", result);
		return;
	}
	...
	while(true)
	{
		...
		switch(op)
		{
			...
			case GET_VOICE_SOURCE:
				get_voice_input();
				break;
			case SET_VOICE_MODULE_SELECT:
				set_voice_module_select();
				break;
			case GET_VOICE_MODULE_SELECT:
				get_voice_module_select();
				break;	
			default:
